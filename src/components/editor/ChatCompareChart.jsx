...
export default function ChatCompareChart({ tickers = [], timeframe = "1M" }) {
  ...
  const handleDragStart = (e) => {
    ...
    ghost.textContent = `ðŸ“Š ${normalized.map(s => s.ticker).join(" + ")}`;
    ...
    >
          ...
          {normalized.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10px] font-bold ${isUp ? "text-gain" : "text-loss"}`}>
      ...
      <div className="px-3 py-1 border-t border-border bg-secondary/30 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
        <span className="text-[9px] text-muted-foreground flex items-center gap-1">
          <GripHorizontal className="w-2.5 h-2.5" /> Drag to report Â· inserts {normalized.length} chart blocks
        </span>
      </div>
    </div>
  );
