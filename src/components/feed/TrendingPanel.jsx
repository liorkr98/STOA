  ...
  return (
    <div className="surface p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-serif text-[14px] text-foreground">Trending Now</h3>
      </div>
        ...
        <div className="space-y-1 mb-4">
          {trendingTickers.map((ticker, i) => (
            <button
              ...
              className="flex items-center gap-2 w-full hover:bg-secondary/60 rounded-tag p-1.5 -mx-1.5 transition-colors text-left"
            >
              <span className="text-xs text-muted-foreground w-3 font-display">{i + 1}</span>
              <span className="text-xs font-display font-medium text-foreground flex-1">${ticker}</span>
            </button>
          ))}
        </div>
      )}
      {trendingTopics.length > 0 && (
        <div className="border-t border-border/60 pt-3">
          <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-2">Top Sectors</p>
          <div className="flex flex-wrap gap-1.5">
            {trendingTopics.map(t => (
              <span key={t} className="text-[10px] px-2 py-0.5 bg-secondary border border-border rounded-tag text-muted-foreground">{t}</span>
            ))}
          </div>
        </div>
      )}
