...
export default function CreatorAnalyticsPage() {
 ...
 className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${range === r.label ? "bg-card text-foreground border border-border" : "text-muted-foreground hover:text-foreground"}`}
              >
                {r.label}
              </button>
            ))}
 ...
 <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-medium text-primary shrink-0 overflow-hidden">
                      {sub.analyst_avatar
 ? <img src={sub.analyst_avatar} alt={`${sub.subscriber_email?.split("@")[0] || "Subscriber"}'s avatar`} className="w-full h-full object-cover" />
